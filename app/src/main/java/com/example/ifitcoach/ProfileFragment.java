...
public class ProfileFragment extends Fragment
{
    ...
    private void OpenConnectionRespondDialog()
    {
        ...
        {
            Picasso.get().load(userImage).placeholder(R.drawable.profile_image_placeholder).into(layoutUserimage);
        }
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
            {
                if(dataSnapshot.exists())
                {
                    if(dataSnapshot.hasChild("userimage"))
                    {
                        ...
                        if(!TextUtils.isEmpty(userImage))
                        {
                            Picasso.get().load(userImage).placeholder(R.drawable.profile_image_placeholder).into(profileUserImage);
                        }
                    }
